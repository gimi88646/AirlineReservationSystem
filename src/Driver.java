...
public class Driver {
        ...
        do {
            try {
                if (airline.user.getSignedInStatus()) {
                    ...
                    System.out.print("1. See Flights\n" + "2. View Bookings\n" + "3. Cancel Booking\n"
                    ...
                    switch (choice) {
                        case 1: {
                            ...
                            if (flight.equals("goBack")) {
                                ...
                                // start taking information.. iske liye method likhna chahiye kyuu user logedin
                                // me bhi yahii kam ho rha hoga
                                // static take info method should be implemented... which returns Arraylist of
                                // string[] each string array represents a passenger
                        ...
                        case 2: {
                            // view Bookings
                            // the user should be able to see his bookings(this query will use username and
                            // date>=today )..
                            // this part should display the user of bookings of today and days forward
                            // for this the date of the system should be taken
                            // there should be query that compares the dates in sql with the current time
                        ...
                        case 3: {
                            // Cancel Booking
                            // this method should be implemented in user class, in user gets to to choose
                            // among his bookings and cancel that booking
                        ...
                        case 4: {
                            // view History
                            // the method should be in user class class.. and calling of method should be
                            // like airline.user.viewHistory
                    ...
                    switch (choice) {
                        ...
                        case 2: {
                            ...
                            // unlike user ki tarah number of passengers and seatType , isme matter nahi
                            // krty
                            ...
                            if (response.equals("notFound")) {
                                ...
                                if (!(mostRecentlyBookedDate == null))
                                    System.out.println("\nFlight " + response + " is only taking bookings till "
                                            + mostRecentlyBookedDate + ".\n");
                                else
                        ...
                        case 3: {
                            ...
                            System.out.print("Enter Cancellation Date: ");
                            String date = input.nextLine();
                            System.out.print("Enter the Reason (for eg, 'Bad Weather', 'Technical Fault'): ");
                            String reason = input.nextLine();

                            // cancel a flight
                            // the admin is supposed to enter a date and to-from , and on that specific date
                            // all the bookings of a to-from gets cancelled
                            break;
                        }
                        case 4: {
                            // log out
                            // when the admin logs out. the admin attribute in airline is assigned to null
                            // again
                            // airline.
                ...
                // once the program starts the neither admin nor user is signed in.. and he gets
                // to see following options
                // if !airline.user.isSigned ..if user is null that means it not yet signed in..
                    ...
                    switch (choice) {
                        ...
                        case 1: {
                            ...
                            // seeFlights();
                            // me chah rha hu ke user ko flightid selection ke sath sath go back ka bhi
                            // option du.. jo ussy le jaega program ke start me
                            // agar me ab user se input lu uski flight id ke according.. tou mujhe kese pata
                            // chale kese input sahi aya he
                            // or agar input sahi aya bhi he tou user ko peeche wale menu me jane ka option
                            // hona chahiye!
                            // agar fligts mili hi nahiii tou kyaa kare?
                            // yahan phr flight id puchengyy..
                            // uske baad phr arraylist banegi people ki...
                            // jitny travellers hongy utni baar information dali jaegi... har individual's
                            // info ki arraylist append hogi people arraylist me
                        ...
                        // sign up module
                        // Sign up information database me store hojaegi or wohii data user.login ke
                        // liye bhi use ki jaegiii
                        ...
                        case 3: {
                            ...
                            // System.out.println("Email Address: "); // a pattern should be declared to
                            // avoid false emails
                            ...
                            // select a username
                            // password
                            // once user sign up krleta he ... tou ussi data ki help se hum uska login
                            // karengy
    ...
    public static String displayBookings(ResultSet resultSet, boolean wantsToCancelBooking) throws SQLException {
        ...
        while (true) {
            // headers
            System.out.println("_".repeat(147) + '\n' + String.format("%-4s", "") + String.format("%-13s", "Flight ID")
                    + String.format("%-15s", "From") + String.format("%-15s", "To") + String.format("%-20s", "Name")
                    + String.format("%-19s", "CNIC") + String.format("%-9s", "Class")
                    + String.format("%-18s", "Departure Time") + String.format("%-18s", "Departure Date")
            ...
            while (resultSet.next()) {
                ...
                // 148
                System.out.println(
                        String.format("%-4s", count + ".") + String.format("%-13s", resultSet.getString("flightId"))
            ...
            if (count == 1) {
                System.out.println(String.format("%89s", "You do not have any Bookings!") + String.format("%59s", '|'));
            }
            System.out.println();
            ...
            if (wantsToCancelBooking) {
                ...
                try {
                    ...
                    if (choice > count || choice < 1)
                        throw new InputOutOfBound("Select From 1 to " + count);
                    if (choice == count) {
                        return "goBack";
                    } else
                        return bookingIds.get(choice - 1);
    ...
    private static String showFlights(ResultSet resultSet) throws SQLException {
        ...
        System.out.println("\nActive Flights from " + resultSet.getString("travelFrom") + " to "
        ...
        if (flightFound) {
            ...
            System.out.println(
                    String.format("%-4s", "") + String.format("%-20s", "Flight ID") + String.format("%-20s", "Time"));
    ...
    private static String seeFlights(ArrayList bookingInfo) {
        ...
        while (true) {
            try {
                ...
                int numberOfPessegers = input.nextInt(); // this number of passengers will be used for comparing with
                                                         // the count of booked flights in database
                System.out.print("Enter Type \n" + "B for Business\n" + "E for Economy\n" + "Your Choice: ");
                char seatType = input.next().charAt(0);

                if (!(seatType == 'B' || seatType == 'E'))
                    throw new InputMismatchException("Please choose between B and E");
    ...
    private static ArrayList<String[]> takeInfoForPassengers(int numberOfPassengers) {
        ...
        for (int i = 0; i < numberOfPassengers; i++) {
            ...
            if (i + 1 < numberOfPassengers) {
                ...
                while (true) {
                    try {
                        System.out.print("1. Continue for next passenger\n" + "2. drop\n" + "choice: ");
                        choice = input.nextInt();
                        if (!(choice == 1 || choice == 2))
                            throw new InputOutOfBound("");
                        break;
                    }
    ...
    public static String inputFrom(ArrayList<String> froms) throws InputOutOfBound, InputMismatchException {
        ...
        int choice = input.nextInt();
        if (choice < 1 || choice > fromsLength)
            throw new InputOutOfBound("Please make sure your input lies in range");
        return froms.get(choice - 1);
    }

    public static String inputDestination(ArrayList<String> destinations)
            throws InputOutOfBound, InputMismatchException {
